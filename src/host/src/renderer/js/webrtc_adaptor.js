...
export class WebRTCAdaptor
{
  ...
  createRdcSession(sessionName,hostId){
   console.log("Create RDC Session called!")
    var jsCmd = {
      command : "createRdcSession",
      rdcSessionName:sessionName,
      rdcHostId : hostId.toString(),
  };
  this.webSocketAdaptor.send(JSON.stringify(jsCmd));

  }

  finishRdcSession(hostId){
    console.log("SET RDC HOST ID CALLED!")
    var jsCmd = {
      rdcHostId:hostId.toString(),
      command : "rdcFinish",
  ...
  //after publishing send stream id to controller so he can play it.
  setRdcStreamIdOnController(controllerSessionId, streamId, hostId){
    console.log("SET RDC STREAM ID ON CONTROLLER!")
    var jsCmd = {
      command : "setRdcStreamId",
      rdcStreamId : streamId.toString(),
      rdcControllerSessionId : controllerSessionId,
      rdcHostId: hostId.toString()
  };
  this.webSocketAdaptor.send(JSON.stringify(jsCmd));
